// Transcrypt'ed from Python, 2019-04-25 02:00:30
...
export var e =  __class__ ('e', [object], {
	...
	get float () {return __get__ (this, function (self, float) {
		self.e.style.float = float;
		return self;
	});},
	get transform () {return __get__ (this, function (self, transform) {
...
export var Arrow =  __class__ ('Arrow', [Div], {
	__module__: __name__,
	get __init__ () {return __get__ (this, function (self, frm, to, args) {
		if (typeof args == 'undefined' || (args != null && args.hasOwnProperty ("__kwargtrans__"))) {;
			var args = dict ({});
		};
		__super__ (Arrow, '__init__') (self);
		var opacity = args.py_get ('opacity', 1);
		self.op (opacity);
		var diff = to.m (frm);
		var l = diff.l ();
		var linewidth = args.py_get ('linewidth', 12);
		var pointwidth = args.py_get ('pointwidth', 36);
		var pointheight = args.py_get ('pointheight', 36);
		self.pointheight = pointheight;
		var color = args.py_get ('color', '#ff7');
		self.h (pointwidth).w (l);
		var lineheight = l - pointheight;
		...
		var rot = Math.asin ((to.y - frm.y) / l);
		if (to.x < frm.x) {
			var rot = Math.PI - rot;
		}
		self.transform (('rotate(' + str ((rot / Math.PI) * 180)) + 'deg)');
		self.a ([self.linediv, self.pointdiv]);
		var shifty = (l / 2) * Math.sin (rot) - pointwidth / 2;
		var shiftx = -(l / 2 - (l / 2) * Math.cos (rot));
