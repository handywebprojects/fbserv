// Transcrypt'ed from Python, 2019-04-20 12:27:59
...
export var Client =  __class__ ('Client', [object], {
	...
	get build () {return __get__ (this, function (self) {
		...
		self.createbotdiv ();
		if (self.doboard) {
			self.mainboard = Board (dict ({'dobook': self.dobook, 'dostoredanalysis': self.dostoredanalysis, 'dostoredauto': self.dostoredauto, 'dogames': self.dogames, 'setposinfo': self.setposinfo, 'dogamepreview': self.dogamepreview, 'background': self.boardbackground, 'autoanalysisdelay': self.autoanalysisdelay, 'maxgames': self.maxgames, 'gamesfilter': self.gamesfilter, 'analysispvlength': self.analysispvlength}));
	...
	get getschemaconfigfromobj () {return __get__ (this, function (self, obj) {
		...
		self.analysispvlength = self.config.py_get ('global/analysispvlength', 4);
